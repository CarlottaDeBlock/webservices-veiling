CREATE TABLE `auctions` (
	...
	`category` enum('Transport','Horeca','Rollend Materiaal','Vastgoed') NOT NULL,
...
CREATE TABLE `lot` (
	...
	`can_bid_higher` boolean NOT NULL DEFAULT true,
...
CREATE TABLE `user` (
	...
	CONSTRAINT `userId_user_email_unique` UNIQUE(`email`)
);
--> statement-breakpoint
ALTER TABLE `auctions` ADD CONSTRAINT `auctions_requester_id_user_user_id_fk` FOREIGN KEY (`requester_id`) REFERENCES `user`(`user_id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `lot` ADD CONSTRAINT `lot_auction_id_auctions_auction_id_fk` FOREIGN KEY (`auction_id`) REFERENCES `auctions`(`auction_id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `user_favorite_lots` ADD CONSTRAINT `user_favorite_lots_lot_id_lot_lot_id_fk` FOREIGN KEY (`lot_id`) REFERENCES `lot`(`lot_id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
